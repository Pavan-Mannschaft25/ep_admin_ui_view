...
//   createdAt: string;
//   updatedAt: string;
//   pricings: ProductPricing[]; // Add pricings array
// }
...
// // Update the productsSlice to handle pricings
// const productsSlice = createSlice({
//   name: "products",
//   initialState,
//   reducers: {
...
//       }
//     },
//     // Replace updateProductPrice with pricing-specific actions
//     addPricing: (
//       state,
...
//       );
//       if (product) {
//         const pricingIndex = product.pricings.findIndex(
//           (p) => p.id === action.payload.pricingId
//         );
//         if (pricingIndex !== -1) {
//           product.pricings[pricingIndex] = action.payload.pricing;
//           product.updatedAt = new Date().toISOString();
//         }
//       }
//     },
...
//       );
//       if (product) {
//         product.pricings = product.pricings.filter(
//           (p) => p.id !== action.payload.pricingId
//         );
//         product.updatedAt = new Date().toISOString();
//       }
...
//       );
//       if (product) {
//         const pricing = product.pricings.find(
//           (p) => p.id === action.payload.pricingId
//         );
//         if (pricing) {
...
const generateProducts = (): Product[] => {
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    // Generate 1-3 pricing options for each product
    const pricingCount = Math.floor(Math.random() * 3) + 1;
    const pricings: ProductPricing[] = [];

    for (let j = 0; j < pricingCount; j++) {
      const quantity = j === 0 ? "1" : j === 1 ? "500" : "250";
      const uom = j === 0 ? "kg" : "g";
