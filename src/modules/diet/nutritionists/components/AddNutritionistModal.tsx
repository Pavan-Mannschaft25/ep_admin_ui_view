// src/modules/nutritionists/components/AddNutritionistModal.tsx
import React, { useState, useEffect, useRef } from "react";
import { useAppDispatch } from "../../../../store/hooks";
import { addNutritionistAsync, Nutritionist } from "../nutritionistSlice";
import { FiX, FiInfo } from "react-icons/fi";
import { toast } from "react-toastify";

interface AddNutritionistModalProps {
  onClose: () => void;
}
...
export default function AddNutritionistModal({
  onClose,
}: AddNutritionistModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    specialization: "",
    experience: 1,
    status: "Pending" as Nutritionist["status"],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use the async thunk for adding a nutritionist
      await dispatch(addNutritionistAsync(formData)).unwrap();
      toast.success("Nutritionist added successfully!");
      onClose();
    } catch (error) {
      toast.error("Failed to add nutritionist. Please try again.");
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-emerald-600 to-teal-700 text-white flex items-center justify-between p-6 border-b border-gray-200">
          <h3 className="text-lg font-semibold">Add New Nutritionist</h3>
          <button
            onClick={onClose}
