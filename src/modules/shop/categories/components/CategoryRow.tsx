...
const CategoryRow = ({ cat }: { cat: Category }) => {
  ...
  // ✅ CONFIRMATION-BASED STATUS TOGGLE
  const handleToggleStatus = () => {
    const newStatus = cat.status === "Active" ? "Inactive" : "Active";
    ...
    dispatch(
      showConfirmationModal({
        ...
        message: `Are you sure you want to ${actionText} the category "${cat.name}"?`,
      ...
      <tr className="border-t hover:bg-gray-50 transition-colors">
        ...
        <td className="p-4">
          <div className="flex gap-2">
            <button
              onClick={openEditModal}
              className="p-2 rounded-lg bg-[#258440] text-white"
            >
              <FiEdit2 />
            </button>
            <button
              onClick={openDeleteModal}
              className="p-2 rounded-lg bg-[#D7201A] text-white"
            >
              <FiTrash2 />
            </button>
          </div>
