// src/modules/fitness/bookings/BookingsPage.tsx
...
export default function BookingsPage() {
  ...
  const filteredBookings = bookings.filter((booking) => {
    const matchesSearch =
      ...
      booking.centerName.toLowerCase().includes(search.toLowerCase()) ||
      booking.phone.includes(search) ||
      booking.id.toString().includes(search) ||
      booking.bookingNo.toLowerCase().includes(search.toLowerCase());

    const matchesCenter =
      centerFilter === "all" || booking.centerName === centerFilter;
    const matchesStatus =
      statusFilter === "all" || booking.status === statusFilter;
    const matchesPayment =
      paymentFilter === "all" || booking.paymentMode === paymentFilter;

    return matchesSearch && matchesCenter && matchesStatus && matchesPayment;
  });
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [search, centerFilter, statusFilter, paymentFilter]);
  ...
  const handleCenterClick = (booking: Booking) => {
    navigate(
      `/center-bookings?centerId=${
        booking.centerId
      }&centerName=${encodeURIComponent(booking.centerName)}`
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by Customer Name, Center, Phone, or Booking ID..."
                  ...
                  {currentBookings.length === 0 ? (
                    ...
                    currentBookings.map((booking) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
                            onClick={() => handleCenterClick(booking)}
                            className="text-sm font-medium text-emerald-600 hover:text-emerald-900 transition-colors duration-150"
                            title={`View all bookings at ${booking.centerName}`}
                          >
                            {booking.centerName}
                          </button>
                        </td>
        ...
        {/* Modals */}
        {showAddModal && (
          <AddBookingModal onClose={() => setShowAddModal(false)} />
        )}
